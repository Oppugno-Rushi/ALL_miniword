...
echo "Checking port 8013..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8013" || lsof -i :8013 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8013 is now free"
else
    echo "⚠️  Port 8013 may still be in use:"
    echo "$PORT_CHECK"
fi
