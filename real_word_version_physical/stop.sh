...
# Check if port 8015 is free
echo "Checking port 8015..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8015" || lsof -i :8015 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8015 is now free"
else
    echo "⚠️  Port 8015 may still be in use:"
    echo "$PORT_CHECK"
fi
