...
class MiniWord {
    constructor() {
        ...
        this.currentToolbar = 'file';
        this.uiUndoStack = [];
        this.uiRedoStack = [];
        this._restoringUiState = false;
        this._maxUiHistory = 50;
        this._preSidebarInteractionState = null;
        this._preChartModalState = null;
        ...
        this.activeInsertedImageWrapper = null;
    ...
    init() {
        ...
        // Inserted image drag / resize / rotate
        this.setupInsertedImageInteractions();
    }
    ...
    setupEventListeners() {
        ...
        // Sidebar control changes should be undoable (UI history), but keep native undo for text inputs.
        this.sidebarContent.addEventListener('pointerdown', (e) => {
            const el = e.target.closest('select, input');
            if (!el) return;
            if (!this._shouldTrackSidebarControl(el)) return;
            if (this._restoringUiState) return;
            this._preSidebarInteractionState = this._buildUiSnapshot();
        }, true);

        this.sidebarContent.addEventListener('focusin', (e) => {
            const el = e.target;
            if (!el) return;
            if (!this._shouldTrackSidebarControl(el)) return;
            if (this._restoringUiState) return;
        ...
        this.sidebarContent.addEventListener('change', (e) => {
            const el = e.target;
            if (!el) return;
            if (!this._shouldTrackSidebarControl(el)) return;
            if (this._restoringUiState) return;

            const snapshot = this._preSidebarInteractionState || this._buildUiSnapshot();
            this._preSidebarInteractionState = null;
            this._pushUiState(snapshot);
        });
        ...
        this.sidebarContent.addEventListener('click', (e) => {
            if (!e.target.closest('.table-size-cell')) return;
            if (this._restoringUiState) return;
            this._pushUiState();
        }, true);

        const chartModal = document.getElementById('chart-insert-modal');
        if (chartModal) {
            chartModal.addEventListener('focusin', () => {
                if (!this._restoringUiState) this._preChartModalState = this._buildUiSnapshot();
            });
            chartModal.addEventListener('change', () => {
                if (this._restoringUiState) return;
                const snapshot = this._preChartModalState || this._buildUiSnapshot();
                this._preChartModalState = null;
                this._pushUiState(snapshot);
            });
        }
        
        // Global Undo/Redo (capture phase so we can skip and let browser handle text undo in inputs)
        document.addEventListener('keydown', (e) => {
            this.handleGlobalUndoRedo(e);
        }, true);
    ...
    switchToolbar(menuType) {
        if (!this._restoringUiState && menuType !== this.currentToolbar) {
            this._pushUiState();
        }
        ...
        this.currentToolbar = menuType;
    }

    _pushUiState(snapshot = null) {
        if (this._restoringUiState) return;
        this.uiRedoStack.length = 0;
        const state = snapshot || this._buildUiSnapshot();
        this.uiUndoStack.push(state);
        if (this.uiUndoStack.length > this._maxUiHistory) this.uiUndoStack.shift();
    }
    ...
    _buildUiSnapshot() {
        const snapshot = {
            ...
            sidebar: this._captureSidebarState()
        };
        if (this.currentPage === 'insert_table') {
            const sel = document.querySelector('#table-size-preview .table-size-cell.selected');
            snapshot.tableSize = sel
                ? { rows: parseInt(sel.dataset.rows, 10), cols: parseInt(sel.dataset.cols, 10) }
                : (this.selectedTableSize || { rows: 3, cols: 3 });
        }
        snapshot.toggles = {
            showInvisibles: this.editor && this.editor.classList.contains('show-invisibles'),
            focusMode: document.body.classList.contains('focus-mode'),
            nightMode: document.body.classList.contains('night-mode')
        };
        const chartModal = document.getElementById('chart-insert-modal');
        if (chartModal && chartModal.style.display !== 'none') {
            snapshot.chartModalOpen = true;
            snapshot.chartModal = this._captureChartModalState();
        } else {
            snapshot.chartModalOpen = false;
        }
        return snapshot;
    }

    _captureChartModalState() {
        const ct = document.getElementById('chart-type');
        const title = document.getElementById('chart-title');
        const ds = document.querySelector('input[name="data-source"]:checked');
        const manual = document.getElementById('manual-data');
        return {
            chartType: ct ? ct.value : 'bar',
            chartTitle: title ? title.value : '',
            dataSource: ds ? ds.value : 'auto',
            manualData: manual ? manual.value : ''
        };
    }

    _restoreChartModalState(state) {
        if (!state) return;
        const ct = document.getElementById('chart-type');
        const title = document.getElementById('chart-title');
        const manual = document.getElementById('manual-data');
        if (ct && state.chartType != null) ct.value = state.chartType;
        if (title && state.chartTitle != null) title.value = state.chartTitle;
        const ds = document.querySelectorAll('input[name="data-source"]');
        ds.forEach(el => { el.checked = (el.value === (state.dataSource || 'auto')); });
        if (manual && state.manualData != null) manual.value = state.manualData;
    }

    _restoreToggles(toggles) {
        if (!toggles) return;
        if (this.editor) {
            if (toggles.showInvisibles) {
                this.editor.classList.add('show-invisibles');
                if (typeof this.addInvisibleMarkers === 'function') this.addInvisibleMarkers();
            } else {
                this.editor.classList.remove('show-invisibles');
                if (typeof this.removeInvisibleMarkers === 'function') this.removeInvisibleMarkers();
            }
        }
        document.body.classList.toggle('focus-mode', !!toggles.focusMode);
        document.body.classList.toggle('night-mode', !!toggles.nightMode);
    }

    _restoreTableSizeSelection(tableSize) {
        if (!tableSize || this.currentPage !== 'insert_table') return;
        ...
        const cell = preview.querySelector(`.table-size-cell[data-rows="${tableSize.rows}"][data-cols="${tableSize.cols}"]`);
        if (cell) {
            cell.classList.add('selected');
            this.selectedTableSize = { rows: tableSize.rows, cols: tableSize.cols };
        }
    }

    _captureSidebarState() {
        const root = this.sidebarContent;
        if (!root) return {};
        const state = {};
        root.querySelectorAll('select, input, textarea').forEach(el => {
            if (!el.id) return;
            if (el.tagName === 'SELECT') {
                state[el.id] = { tag: 'select', value: el.value };
                return;
            }
            if (el.tagName === 'TEXTAREA') {
                state[el.id] = { tag: 'textarea', value: el.value };
                return;
            }
            if (el.tagName === 'INPUT') {
                const type = (el.type || 'text').toLowerCase();
                if (type === 'checkbox' || type === 'radio') {
                    state[el.id] = { tag: 'input', type, checked: !!el.checked };
                } else {
                    state[el.id] = { tag: 'input', type, value: el.value };
                }
            }
        });
        const radioNames = new Set();
        root.querySelectorAll('input[type="radio"][name]').forEach(el => {
            const name = el.getAttribute('name');
            if (!name || radioNames.has(name)) return;
            radioNames.add(name);
    ...
    _restoreSidebarState(sidebarState) {
        if (!sidebarState) return;
        const root = this.sidebarContent;
        Object.keys(sidebarState).forEach(key => {
            if (key.startsWith('radio_')) {
                const name = key.slice(6);
                const value = sidebarState[key];
                const group = root ? root.querySelectorAll('input[type="radio"][name="' + name + '"]') : document.querySelectorAll('input[type="radio"][name="' + name + '"]');
                group.forEach(radio => {
    ...
    _shouldTrackSidebarControl(el) {
        if (!el) return false;
        if (el.tagName === 'SELECT') return true;
        if (el.tagName === 'TEXTAREA') return true;
        if (el.tagName !== 'INPUT') return false;
        const t = (el.type || 'text').toLowerCase();
        return t === 'checkbox' || t === 'radio' || t === 'text' || t === 'number' || t === 'url' || t === 'email';
    }
    ...
    uiUndo() {
        if (this.uiUndoStack.length === 0) return;
        const state = this.uiUndoStack.pop();
        this.uiRedoStack.push(this._buildUiSnapshot());
        this._restoringUiState = true;
        this.switchToolbar(state.toolbar);
        this.showPage(state.page);
        this._restoreSidebarState(state.sidebar);
        if (state.tableSize) this._restoreTableSizeSelection(state.tableSize);
        if (state.toggles) this._restoreToggles(state.toggles);
        if (state.chartModalOpen) {
            if (typeof this.showChartInsertModal === 'function') this.showChartInsertModal();
            if (state.chartModal) this._restoreChartModalState(state.chartModal);
        } else {
            if (typeof this.closeChartModal === 'function') this.closeChartModal();
        }
        this._restoringUiState = false;
    }

    uiRedo() {
        if (this.uiRedoStack.length === 0) return;
        const state = this.uiRedoStack.pop();
        this.uiUndoStack.push(this._buildUiSnapshot());
        this._restoringUiState = true;
        this.switchToolbar(state.toolbar);
        this.showPage(state.page);
        this._restoreSidebarState(state.sidebar);
    ...
    handleGlobalUndoRedo(e) {
        const isCtrl = e.ctrlKey || e.metaKey;
        const isUndo = isCtrl && e.key === 'z' && !e.shiftKey;
        const isRedo = isCtrl && (e.key === 'y' || (e.key === 'z' && e.shiftKey));
        if (!isUndo && !isRedo) return;
        if (this.editor.contains(e.target)) return;
        const t = e.target;
        const isTextOrTextarea = (t.tagName === 'TEXTAREA') || (t.tagName === 'INPUT' && !/^(checkbox|radio)$/i.test(t.type || ''));
        if (isTextOrTextarea) return;
        const chartModalEl = document.getElementById('chart-insert-modal');
        if (chartModalEl && chartModalEl.style.display !== 'none' && chartModalEl.contains(e.target)) {
            e.preventDefault();
            if (isUndo) this.uiUndo();
            else this.uiRedo();
            return;
        }
        if (e.target.isContentEditable && e.target !== this.editor && !this.editor.contains(e.target)) return;
        e.preventDefault();
        if (isUndo) this.uiUndo();
        else this.uiRedo();
    }
    ...
    showPage(pageId) {
        if (!this._restoringUiState && pageId !== this.currentPage) {
            this._pushUiState();
        }
        this.currentPage = pageId;
        
        // Insert Chart: show sidebar only (chart settings in sidebar, no modal)
    ...
    renderPage(pageId) {
        ...
        if (pageId !== 'bold' && this._boldSelectionListener) {
            document.removeEventListener('selectionchange', this._boldSelectionListener);
            this._boldSelectionListener = null;
        }
        const pageContent = this.getPageContent(pageId);
        console.log('Page content:', pageContent); // Debug log
        // Sidebar top header: always "Function Panel" for every button (all 62). No icon/function name here—
        // the function name (New Document, Open Document, Save, Undo, etc.) appears only once, in the content below the blue line.
        this.sidebarTitle.textContent = 'Function Panel';
        ...
        if (pageId === 'insert_image') {
            this.ensureInsertImageMonitorUI();
            this.updateInsertImageInfoPanel();
        }
    }
    ...
    getPageContent(pageId) {
        const pages = {
            ...
            'bold': {
                ...
                content: `
                    <div class="page-content">
                        <div class="bold-option-group">
                            <p style="margin: 0 0 4px 0; font-size: 13px; color: #555;">
                                Bold the selected text.
                            </p>
                            <button id="bold-text-btn" class="btn btn-primary" onclick="miniWord.toggleBold()">Apply Bold to Text (Ctrl+B)</button>
                        </div>
                        <div class="bold-option-group">
                            <p style="margin: 8px 0 4px 0; font-size: 13px; color: #555;">
                                Bold only the borders of the current table. Click inside a table cell first so MiniWord knows which table to update.
                            </p>
                            <button id="bold-table-border-btn" class="btn btn-primary" onclick="miniWord.boldTableBorderOnly()">Bold table border only</button>
                            <p style="margin: 8px 0 4px 0; font-size: 13px; color: #555;">
                                Bold all text and borders in the current table. Also requires the cursor to be inside the table.
                            </p>
                            <button id="bold-whole-table-btn" class="btn btn-primary" onclick="miniWord.boldWholeTable()">Bold whole table</button>
                        </div>
                        <div class="button-group">
            ...
            'insert_chart': {
                title: 'Insert Chart',
                content: `
                    <div class="page-content insert-chart-sidebar">
                        <div class="input-group">
                        ...
                        <div class="input-group" id="detected-tables-section">
                            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
                                <label style="margin: 0;">Detected Tables:</label>
                                <button type="button" class="btn btn-sm" onclick="miniWord.detectTablesInDocument()" style="padding: 4px 8px; font-size: 12px;">Refresh</button>
                            </div>
                        ...
                        <div class="button-group" style="margin-top: 16px; display: flex; gap: 8px;">
                            <button type="button" class="btn btn-secondary" onclick="miniWord.showPage('insert')">Cancel</button>
                            <button type="button" class="btn btn-primary" id="insert-chart-btn" onclick="miniWord.insertSelectedChart()">Insert Chart</button>
                        </div>
                    </div>
                `
            },
            ...
            'help_document': {
                ...
                content: `
                    <div class="page-content">
                        <div class="help-doc-search-wrap">
                            <input type="text" id="help-doc-search" class="help-doc-search-input" placeholder="Search help..." aria-label="Search help document">
                        </div>
                        <div id="help-document-container" class="help-doc-container">
            ...
            'insert_image': {
                ...
                content: `
                    <div class="page-content">
                        ...
                        <div class="insert-image-monitor" id="insert-image-monitor">
                            <h4>Current Image on Canvas</h4>
                            <div class="insert-image-monitor-row">
                                <span>Selected:</span>
                                <strong id="insert-image-info-selected">No image selected</strong>
                            </div>
                            <div class="insert-image-monitor-row">
                                <span>Angle:</span>
                                <strong id="insert-image-info-angle">-</strong>
                            </div>
                            <div class="insert-image-monitor-row">
                                <span>Size:</span>
                                <strong id="insert-image-info-size">-</strong>
                            </div>
                            <div class="insert-image-monitor-row">
                                <span>Position:</span>
                                <strong id="insert-image-info-position">-</strong>
                            </div>
                            <div class="insert-image-control-group">
                                <label>Angle Control</label>
                                <div class="insert-image-control-row">
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.adjustSelectedImageAngle(-1)">-1deg</button>
                                    <input type="number" id="insert-image-angle-input" step="0.1" value="0">
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.adjustSelectedImageAngle(1)">+1deg</button>
                                    <button class="btn btn-primary insert-image-apply-btn" onclick="miniWord.applySelectedImageAngle()">Apply</button>
                                </div>
                            </div>
                            <div class="insert-image-control-group">
                                <label>Size Control (px)</label>
                                <div class="insert-image-control-row insert-image-control-row-two-inputs">
                                    <input type="number" id="insert-image-width-input" min="20" step="1" placeholder="Width">
                                    <input type="number" id="insert-image-height-input" min="20" step="1" placeholder="Height">
                                </div>
                                <div class="insert-image-control-row">
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.adjustSelectedImageSize(-10)">-10</button>
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.adjustSelectedImageSize(10)">+10</button>
                                    <button class="btn btn-primary insert-image-apply-btn" onclick="miniWord.applySelectedImageSize()">Apply Size</button>
                                </div>
                            </div>
                            <div class="insert-image-control-group">
                                <label>Position Control (px)</label>
                                <div class="insert-image-control-row insert-image-control-row-two-inputs">
                                    <input type="number" id="insert-image-x-input" step="1" placeholder="X">
                                    <input type="number" id="insert-image-y-input" step="1" placeholder="Y">
                                </div>
                                <div class="insert-image-control-row">
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.nudgeSelectedImage(-5, 0)">Left</button>
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.nudgeSelectedImage(5, 0)">Right</button>
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.nudgeSelectedImage(0, -5)">Up</button>
                                    <button class="btn btn-secondary insert-image-adjust-btn" onclick="miniWord.nudgeSelectedImage(0, 5)">Down</button>
                                    <button class="btn btn-primary insert-image-apply-btn" onclick="miniWord.applySelectedImagePosition()">Apply Pos</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    content: `
                        <div class="page-content">
                            <div style="text-align: center; margin: 20px 0;">
                                ${iconSrc ? `<img src="${iconSrc}" alt="${iconAlt}" style="width: 32px; height: 32px; margin-bottom: 8px;">` : ''}
                            </div>
    ...
    bindPageEvents(pageId) {
        ...
        switch (pageId) {
            ...
            case 'help_document':
                ...
                break;
            case 'bold':
                this.updateBoldSidebarState();
                this._boldSelectionListener = () => { if (this.currentPage === 'bold') this.updateBoldSidebarState(); };
                document.addEventListener('selectionchange', this._boldSelectionListener);
                break;
        }
    }

    async loadHelpDocument() {
        const container = document.getElementById('help-document-container');
        if (!container) {
            return;
        }

        container.innerHTML = '<p>Loading help document...</p>';

        try {
            const response = await fetch('explainations_for_botton.json');
            if (!response.ok) {
                throw new Error(`HTTP ${response.status}`);
            }
            const data = await response.json();
            const doc = data.MiniWord_Button_Documentation || {};
            const sections = doc.sections || {};

            let html = '';
            Object.keys(sections).forEach(sectionName => {
                const section = sections[sectionName];
                html += '<div class=\"help-doc-section\">';
                html += `<h3>${sectionName}</h3>`;
                if (section.description) {
                    html += `<p class=\"help-doc-section-desc\">${section.description}</p>`;
                }

                if (Array.isArray(section.buttons)) {
                    section.buttons.forEach(btn => {
                        html += '<div class=\"help-doc-button\">';
                        html += `<h4>${btn.name || btn.id || ''}</h4>`;
                        if (btn.shortcut) {
                            html += `<p class=\"help-doc-shortcut\"><strong>Shortcut:</strong> ${btn.shortcut}</p>`;
                        }
                        if (Array.isArray(btn.usage) && btn.usage.length) {
                            html += '<ul class=\"help-doc-usage\">';
                            btn.usage.forEach(step => {
                                html += `<li>${step}</li>`;
                            });
                            html += '</ul>';
                        }
            ...
            container.innerHTML = html || '<p>No help content available.</p>';
            this.bindHelpDocumentSearch();
        } catch (err) {
            console.error('Failed to load help document', err);
            container.innerHTML = '<p style=\"color: #d32f2f;\">Failed to load help document.</p>';
        }
    }

    bindHelpDocumentSearch() {
        const searchInput = document.getElementById('help-doc-search');
        const container = document.getElementById('help-document-container');
        if (!searchInput || !container) return;

        const removeHighlights = () => {
            container.querySelectorAll('.help-doc-highlight').forEach(span => {
        ...
        const highlightInElement = (el, q) => {
            ...
            textNodes.forEach(textNode => {
                ...
                while ((idx = lower.indexOf(q, pos)) !== -1) {
                    parts.push({ type: 'text', value: text.substring(pos, idx) });
                    parts.push({ type: 'match', value: text.substring(idx, idx + q.length) });
                    pos = idx + q.length;
                }
                ...
                parts.forEach(p => {
                    if (p.type === 'text' && p.value) parent.insertBefore(document.createTextNode(p.value), textNode);
                    if (p.type === 'match') {
                        const span = document.createElement('span');
                        span.className = 'help-doc-highlight';
        ...
        const runFilter = () => {
            ...
            removeHighlights();
            const sections = container.querySelectorAll('.help-doc-section');
            sections.forEach(section => {
                const buttons = section.querySelectorAll('.help-doc-button');
            ...
            if (q) {
                sections.forEach(section => {
                    if (section.style.display !== 'none') highlightInElement(section, q);
                });
            }
        };
    ...
    showChartInsertModal() {
        if (!this._restoringUiState) this._pushUiState();
...
[Your Title]`;
    ...
    getSelectionTable() {
        ...
        for (let n = node; n && n !== this.editor; n = n.parentElement) {
            if (n.tagName === 'TABLE') return n;
        }
        return null;
    }

    updateBoldSidebarState() {
        const textBtn = document.getElementById('bold-text-btn');
        const borderBtn = document.getElementById('bold-table-border-btn');
        const wholeBtn = document.getElementById('bold-whole-table-btn');
        if (!textBtn || !borderBtn || !wholeBtn) return;
        const inTable = !!this.getSelectionTable();
        textBtn.disabled = inTable;
        textBtn.classList.toggle('bold-option-disabled', inTable);
        borderBtn.disabled = !inTable;
        borderBtn.classList.toggle('bold-option-disabled', !inTable);
        wholeBtn.disabled = !inTable;
        wholeBtn.classList.toggle('bold-option-disabled', !inTable);
    }

    boldTableBorderOnly() {
        const table = this.getSelectionTable();
        if (!table) {
            this.showMessage('Please select a cell or place cursor inside a table first.');
            return;
        }
        const borderStyle = '2px solid #333';
        table.style.border = borderStyle;
        table.querySelectorAll('td, th').forEach(cell => {
            cell.style.border = borderStyle;
        });
        this.showMessage('Table border set to bold.');
    }
    ...
    boldWholeTable() {
        ...
        const borderStyle = '3px solid #333';
        table.style.border = borderStyle;
        table.querySelectorAll('td, th').forEach(cell => {
            cell.style.border = borderStyle;
            cell.style.fontWeight = 'bold';
        });
        this.showMessage('Whole table set to bold (borders and text).');
    }
    ...
    setupChartModalEvents() {
        const chartControlsRoot = this.getActiveChartControlsRoot();
        if (!chartControlsRoot) return;

        // Handle data source radio buttons
        const dataSourceRadios = chartControlsRoot.querySelectorAll('input[name="data-source"]');
        const detectedTablesSection = chartControlsRoot.querySelector('#detected-tables-section');
        const manualDataSection = chartControlsRoot.querySelector('#manual-data-section');
        ...
        const chartTypeSelect = chartControlsRoot.querySelector('#chart-type');
        if (chartTypeSelect) {
    ...
    getActiveChartControlsRoot() {
        const sidebarChartRoot = this.sidebarContent
            ? this.sidebarContent.querySelector('.insert-chart-sidebar')
            : null;
        if (sidebarChartRoot) {
            return sidebarChartRoot;
        }

        const chartModal = document.getElementById('chart-insert-modal');
        if (chartModal && window.getComputedStyle(chartModal).display !== 'none') {
            return chartModal;
        }
    ...
    detectTablesInDocument() {
        const detectedTablesList = (this.sidebarContent && this.sidebarContent.querySelector('#detected-tables-list'))
            || document.getElementById('detected-tables-list');
        if (!detectedTablesList) {
            console.error('detected-tables-list element not found!');
            return;
        }

        const editor = document.getElementById('editor');
        let tables = editor ? Array.from(editor.querySelectorAll('table')) : [];
        if (tables.length === 0) {
            const editorContainer = document.querySelector('.editor-container');
            if (editorContainer) {
                tables = Array.from(editorContainer.querySelectorAll('table'));
            }
        }
        if (tables.length === 0) {
            const allTables = document.querySelectorAll('table');
            ...
            const modal = document.getElementById('chart-insert-modal');
            tables = Array.from(allTables).filter(t => {
                if (sidebar && sidebar.contains(t)) return false;
                if (modal && modal.contains(t)) return false;
                return true;
            });
        }
    ...
    selectTableForChart(table, tableData, index) {
        ...
        chartControlsRoot.querySelectorAll('.table-item').forEach(item => {
            item.style.backgroundColor = 'white';
            item.style.borderColor = '#e1e5e9';
        });
        
        const selectedItem = chartControlsRoot.querySelectorAll('.table-item')[index];
    ...
    updateChartPreview() {
        ...
        const chartType = chartControlsRoot.querySelector('#chart-type')?.value || 'bar';
        const chartTitle = chartControlsRoot.querySelector('#chart-title')?.value || 'Chart';
    ...
    insertSelectedChart() {
        ...
        const chartControlsRoot = this.getActiveChartControlsRoot();
        if (!chartControlsRoot) {
            this.showMessage('Chart controls are not available');
            return;
        }

        const chartType = chartControlsRoot.querySelector('#chart-type')?.value || 'bar';
        const chartTitle = chartControlsRoot.querySelector('#chart-title')?.value || 'Chart';
        const dataSource = chartControlsRoot.querySelector('input[name="data-source"]:checked')?.value || 'auto';
        
        let chartData;
        
        if (dataSource === 'manual') {
            const manualData = chartControlsRoot.querySelector('#manual-data');
            ...
            const selectedTableItem = chartControlsRoot.querySelector('#detected-tables-list .table-item[style*="background-color: #e3f2fd"]');
            if (!selectedTableItem && !this.selectedTableData) {
                ...
                const refreshedSelectedItem = chartControlsRoot.querySelector('#detected-tables-list .table-item[style*="background-color: #e3f2fd"]');
...
<body>
    ...
    insertImage() {
        ...
        reader.onload = (e) => {
            const imageHTML = `
                <div class="inserted-image-container inserted-image-wrapper" contenteditable="false" style="
                    position: relative;
                ...
                ">
                    <img src="${e.target.result}" 
                         ...
                         style="
                            ...
                            box-sizing: border-box;
                        "
                        class="inserted-image" draggable="false">
                    <span class="resize-handle resize-se" data-handle="se"></span>
                    <span class="resize-handle resize-sw" data-handle="sw"></span>
                    <span class="resize-handle resize-ne" data-handle="ne"></span>
                    <span class="resize-handle resize-nw" data-handle="nw"></span>
    ...
    setupInsertedImageInteractions() {
        if (!this.editor) return;
        this.editor.addEventListener('mousedown', (e) => {
            const wrapper = e.target.closest('.inserted-image-wrapper');
            if (!wrapper) return;
            const img = wrapper.querySelector('.inserted-image');
            if (!img) return;
            this.activeInsertedImageWrapper = wrapper;
            this.updateInsertImageInfoPanel(wrapper);
            ...
            if (resizeHandle) {
                ...
                const startW = imgRect.width, startH = imgRect.height;
                const startLeft = parseFloat(wrapper.style.left) || 0, startTop = parseFloat(wrapper.style.top) || 0;
                const move = (e2) => {
                    const dx = e2.clientX - startX, dy = e2.clientY - startY;
                    let newW = startW, newH = startH, dLeft = 0, dTop = 0;
                    if (handle === 'se') { newW = startW + dx; newH = startH + dy; }
                    else if (handle === 'sw') { newW = Math.max(20, startW - dx); newH = startH + dy; dLeft = startW - newW; }
                    else if (handle === 'ne') { newW = startW + dx; newH = Math.max(20, startH - dy); dTop = startH - newH; }
                    else if (handle === 'nw') { newW = Math.max(20, startW - dx); newH = Math.max(20, startH - dy); dLeft = startW - newW; dTop = startH - newH; }
                    newW = Math.max(20, newW); newH = Math.max(20, newH);
                    img.style.width = newW + 'px'; img.style.height = newH + 'px'; img.style.maxWidth = 'none';
                    wrapper.style.left = (startLeft + dLeft) + 'px'; wrapper.style.top = (startTop + dTop) + 'px';
                    this.updateInsertImageInfoPanel(wrapper);
                };
                const up = () => {
            ...
            if (rotateHandle) {
                ...
                const tr = (img.style.transform || '').match(/rotate\(([-\d.]+)deg\)/);
                const startRotate = tr ? parseFloat(tr[1]) : 0;
                const move = (e2) => {
                    const curAngle = Math.atan2(e2.clientY - cy, e2.clientX - cx);
            ...
            const move = (e2) => {
                const dx = e2.clientX - startX, dy = e2.clientY - startY;
                wrapper.style.left = (startLeft + dx) + 'px'; wrapper.style.top = (startTop + dy) + 'px';
                this.updateInsertImageInfoPanel(wrapper);
            };
            const up = () => {
                document.removeEventListener('mousemove', move);
    ...
    ensureInsertImageMonitorUI() {
        if (!this.sidebarContent) return;
        const pageRoot = this.sidebarContent.querySelector('.page-content');
        if (!pageRoot) return;

        let monitor = pageRoot.querySelector('#insert-image-monitor');
        if (!monitor) {
            monitor = document.createElement('div');
            monitor.className = 'insert-image-monitor';
            monitor.id = 'insert-image-monitor';
            monitor.innerHTML = `
                <h4>Current Image on Canvas</h4>
                <div class="insert-image-monitor-row">
                    <span>Selected:</span>
            ...
            `;
            const buttonGroup = pageRoot.querySelector('.button-group');
            if (buttonGroup) {
                buttonGroup.insertAdjacentElement('afterend', monitor);
            } else {
                pageRoot.appendChild(monitor);
            }
        }

        monitor.style.display = 'block';
        monitor.style.marginTop = '14px';
        monitor.style.padding = '12px';
        monitor.style.border = '1px solid #dbe8f8';
        monitor.style.borderRadius = '6px';
        monitor.style.background = '#f7fbff';
    }

    updateInsertImageInfoPanel(targetWrapper = this.activeInsertedImageWrapper) {
        this.ensureInsertImageMonitorUI();
        const selectedEl = document.getElementById('insert-image-info-selected');
        const angleEl = document.getElementById('insert-image-info-angle');
        const sizeEl = document.getElementById('insert-image-info-size');
        const positionEl = document.getElementById('insert-image-info-position');
        const angleInput = document.getElementById('insert-image-angle-input');
        const widthInput = document.getElementById('insert-image-width-input');
        const heightInput = document.getElementById('insert-image-height-input');
        const xInput = document.getElementById('insert-image-x-input');
        const yInput = document.getElementById('insert-image-y-input');

        if (!selectedEl || !angleEl || !sizeEl || !positionEl) return;

        if (!targetWrapper || !document.body.contains(targetWrapper)) {
            selectedEl.textContent = 'No image selected';
            angleEl.textContent = '-';
            sizeEl.textContent = '-';
        ...
        const img = targetWrapper.querySelector('.inserted-image');
        if (!img) {
            selectedEl.textContent = 'No image selected';
            angleEl.textContent = '-';
            sizeEl.textContent = '-';
        ...
        this.activeInsertedImageWrapper = targetWrapper;
        selectedEl.textContent = 'Image selected';

        const rotateMatch = (img.style.transform || '').match(/rotate\(([-\d.]+)deg\)/);
        const angle = rotateMatch ? parseFloat(rotateMatch[1]) : 0;
        angleEl.textContent = `${angle.toFixed(1)}deg`;
        if (angleInput) angleInput.value = angle.toFixed(1);

        const imageRect = img.getBoundingClientRect();
        sizeEl.textContent = `${imageRect.width.toFixed(1)} x ${imageRect.height.toFixed(1)} px`;
        if (widthInput) widthInput.value = imageRect.width.toFixed(1);
        if (heightInput) heightInput.value = imageRect.height.toFixed(1);

        const wrapperRect = targetWrapper.getBoundingClientRect();
        const editorRect = this.editor ? this.editor.getBoundingClientRect() : null;
        let x = parseFloat(targetWrapper.style.left) || 0;
        let y = parseFloat(targetWrapper.style.top) || 0;
        if (editorRect) {
            x = wrapperRect.left - editorRect.left + (this.editor.scrollLeft || 0);
            y = wrapperRect.top - editorRect.top + (this.editor.scrollTop || 0);
        }
        positionEl.textContent = `x: ${x.toFixed(1)} px, y: ${y.toFixed(1)} px`;
        if (xInput) xInput.value = x.toFixed(1);
        if (yInput) yInput.value = y.toFixed(1);
    }

    getActiveInsertedImageElements() {
        const wrapper = this.activeInsertedImageWrapper;
    ...
    getCurrentImageCanvasPosition(wrapper) {
    ...
    adjustSelectedImageAngle(delta) {
        const active = this.getActiveInsertedImageElements();
        if (!active) return;
        const { img, wrapper } = active;
        const tr = (img.style.transform || '').match(/rotate\(([-\d.]+)deg\)/);
        const currentAngle = tr ? parseFloat(tr[1]) : 0;
        img.style.transform = `rotate(${(currentAngle + delta).toFixed(1)}deg)`;
        this.updateInsertImageInfoPanel(wrapper);
    }

    applySelectedImageAngle() {
        const active = this.getActiveInsertedImageElements();
        const angleInput = document.getElementById('insert-image-angle-input');
        if (!active || !angleInput) return;
        const nextAngle = parseFloat(angleInput.value);
        if (Number.isNaN(nextAngle)) return;
        active.img.style.transform = `rotate(${nextAngle.toFixed(1)}deg)`;
        this.updateInsertImageInfoPanel(active.wrapper);
    }

    adjustSelectedImageSize(deltaPx) {
        const active = this.getActiveInsertedImageElements();
        if (!active) return;
        const { img, wrapper } = active;
        const rect = img.getBoundingClientRect();
        const nextW = Math.max(20, rect.width + deltaPx);
        const nextH = Math.max(20, rect.height + deltaPx);
    ...
    applySelectedImageSize() {
        const active = this.getActiveInsertedImageElements();
        const widthInput = document.getElementById('insert-image-width-input');
        const heightInput = document.getElementById('insert-image-height-input');
        if (!active || !widthInput || !heightInput) return;

        const widthValue = parseFloat(widthInput.value);
        const heightValue = parseFloat(heightInput.value);
        if (Number.isNaN(widthValue) || Number.isNaN(heightValue)) return;

        const { img, wrapper } = active;
        img.style.width = `${Math.max(20, widthValue)}px`;
        img.style.height = `${Math.max(20, heightValue)}px`;
        img.style.maxWidth = 'none';
        this.updateInsertImageInfoPanel(wrapper);
    }

    nudgeSelectedImage(dx, dy) {
        const active = this.getActiveInsertedImageElements();
        if (!active) return;
    ...
    applySelectedImagePosition() {
        ...
        if (Number.isNaN(targetX) || Number.isNaN(targetY)) return;

        const { wrapper } = active;
        const currentPos = this.getCurrentImageCanvasPosition(wrapper);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Persistent label below each toolbar icon (no hover)
    document.querySelectorAll('.toolbar-btn').forEach(function(btn) {
        const labelText = btn.getAttribute('aria-label') || btn.getAttribute('title') || '';
        if (labelText && !btn.querySelector('.toolbar-btn-label')) {
            const span = document.createElement('span');
