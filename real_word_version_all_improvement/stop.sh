...
# Check if port 8016 is free
echo "Checking port 8016..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8016" || lsof -i :8016 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8016 is now free"
else
    echo "⚠️  Port 8016 may still be in use:"
    echo "$PORT_CHECK"
fi
