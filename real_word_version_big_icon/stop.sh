...
# Check if port 8009 is free
echo "Checking port 8009..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8009" || lsof -i :8009 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8009 is now free"
else
    echo "⚠️  Port 8009 may still be in use:"
    echo "$PORT_CHECK"
fi
