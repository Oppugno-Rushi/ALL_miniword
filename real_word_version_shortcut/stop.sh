...
# Check if port 8014 is free
echo "Checking port 8014..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8014" || lsof -i :8014 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8014 is now free"
else
    echo "⚠️  Port 8014 may still be in use:"
    echo "$PORT_CHECK"
fi
