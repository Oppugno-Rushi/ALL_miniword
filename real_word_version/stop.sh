...
# Check if port 8008 is free
echo "Checking port 8008..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8008" || lsof -i :8008 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8008 is now free"
else
    echo "⚠️  Port 8008 may still be in use:"
    echo "$PORT_CHECK"
fi
