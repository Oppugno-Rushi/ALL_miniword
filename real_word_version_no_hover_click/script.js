...
class MiniWord {
    ...
    setupEventListeners() {
        ...
        // Floating popover for icon explanation (one popover, follows clicked icon)
        const iconPopover = document.createElement('div');
        iconPopover.className = 'toolbar-icon-popover';
        iconPopover.innerHTML = '<span class="toolbar-icon-popover-text"></span><button type="button" class="toolbar-icon-popover-close" aria-label="Close">×</button>';
        document.body.appendChild(iconPopover);
        this.iconPopover = iconPopover;
        this.iconPopoverButton = null;
        const popoverTextEl = iconPopover.querySelector('.toolbar-icon-popover-text');
        const popoverCloseBtn = iconPopover.querySelector('.toolbar-icon-popover-close');
        popoverCloseBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            iconPopover.classList.remove('toolbar-icon-popover-visible');
            this.iconPopoverButton = null;
        });
        ...
        document.querySelectorAll('.toolbar-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                // Show floating popover with icon name; hide previous one
                if (button) {
                    const text = button.getAttribute('aria-label') || button.getAttribute('title') || '';
                    if (text) {
                        popoverTextEl.textContent = text;
                        const rect = button.getBoundingClientRect();
                        const content = button.closest('.toolbar-content');
                        const isFirstInSection = content && content.querySelector('.toolbar-btn') === button;
                        if (isFirstInSection) {
                            iconPopover.style.left = `${Math.max(8, rect.left)}px`;
                            iconPopover.style.transform = 'none';
                        } else {
                            iconPopover.style.left = `${rect.left + rect.width / 2}px`;
                            iconPopover.style.transform = 'translateX(-50%)';
                        }
                        iconPopover.style.top = `${rect.bottom + 6}px`;
                        iconPopover.classList.add('toolbar-icon-popover-visible');
                        this.iconPopoverButton = button;
                    }
                }
    ...
    switchToolbar(menuType) {
        // Hide floating icon popover when switching function tabs
        if (this.iconPopover) {
            this.iconPopover.classList.remove('toolbar-icon-popover-visible');
            this.iconPopoverButton = null;
        }
