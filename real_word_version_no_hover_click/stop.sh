...
# Check if port 8010 is free
echo "Checking port 8010..."
PORT_CHECK=$(netstat -tlnp 2>/dev/null | grep ":8010" || lsof -i :8010 2>/dev/null)
if [ -z "$PORT_CHECK" ]; then
    echo "✅ Port 8010 is now free"
else
    echo "⚠️  Port 8010 may still be in use:"
    echo "$PORT_CHECK"
fi
